...
import { Icon as ArrowDownRight, Icon as ArrowUpRight, Icon as Plus, Icon as Wallet } from '../components/Icon'
    ...
    return (
      <div className="space-y-7">
        ...
        <div className="grid gap-6 xl:grid-cols-5">
          <section className="rounded-card border border-line bg-surface p-6 xl:col-span-2"><h2 className="text-lg font-bold">Spending by category</h2><div className="mt-6 space-y-4">{Object.keys(categories).length === 0 ? <p className="text-sm text-muted">No expenses recorded yet.</p> : Object.keys(categories).map((category) => <div key={category}><div className="flex justify-between text-sm"><span>{category}</span><strong>{this.currency(categories[category])}</strong></div><div className="mt-2 h-2 rounded-full bg-slate-100"><div className="h-2 w-full rounded-full bg-accent" /></div></div>)}</div></section>
