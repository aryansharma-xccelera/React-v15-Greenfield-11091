...
import { Icon as ArrowLeft, Icon as Save } from '../components/Icon'
...
interface FormState extends TransactionInput { categories: Category[]; attempted: boolean; saving: boolean; loaded: boolean; success: string }

export class TransactionForm extends React.Component<RouteComponentProps<RouteParams>, FormState> {
  public state: FormState = { amount: 0, type: 'Expense', category: '', date: '', description: '', categories: [], attempted: false, saving: false, loaded: false, success: '' }
  ...
  private error(field: string) {
    ...
    if (field === 'category' && !this.state.category) return 'Choose a category.'
    if (field === 'date' && !this.state.date) return 'Choose a date.'
    return ''
  }
  ...
  private submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (this.state.amount <= 0 || !this.state.category || !this.state.date) return
  ...
  public render() {
    ...
    const isValid = this.state.amount > 0 && Boolean(this.state.category) && Boolean(this.state.date)
